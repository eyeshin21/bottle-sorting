...
namespace Anvil
{
    public class Monitor4AnimationCurve : EditorWindow
    {
        ...
        string GetLastRelativeName(HierarchyChangedDetecter.EChangeType type,
            HierarchyChangedDetecter.HierarchySnapshot snapshot)
        {
        ...
        void MonitorGameObject(HierarchyChangedDetecter.EChangeType type,
            HierarchyChangedDetecter.HierarchySnapshot snapshot)
        {
            ...
            if (anims != null && anims.Length > 0)
            {
                foreach (AnimationClip ac in anims)
                {
                    ...
                    {
                        if (objectCurveBinding[i].path.CompareTo(oldPath) == 0 ||
                            objectCurveBinding[i].path.StartsWith(oldPath + "/"))
                        {
                            ...
                            ObjectReferenceKeyframe[] keyframes =
                                AnimationUtility.GetObjectReferenceCurve(ac, objectCurveBinding[i]);
                    ...
                    {
                        if (curveDataBinding[i].path.CompareTo(oldPath) == 0 ||
                            curveDataBinding[i].path.StartsWith(oldPath + "/"))
                        {
        ...
        {
            _monitorTransform =
                EditorGUILayout.ObjectField("Monitor", _monitorTransform, typeof(Transform), true) as Transform;
        }
