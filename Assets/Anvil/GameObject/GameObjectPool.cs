...
namespace Anvil
{
    public static class GameObjectPool
    {
        ...
        public static T CreateObject<T>(Transform parent, GameObject prefab, bool copyParentPos = true,
            bool worldPositionStays = false, bool resetScale = true, Action<GameObject> onInstantiate = null)
            where T : MonoBehaviour
        {
            GameObject gameObject = CreateObject(parent, prefab, copyParentPos, worldPositionStays, resetScale,
                onInstantiate);
            if (gameObject == null)
                return null;
            ...
            Debug.LogError($"[GameObjectPool] Created object ({gameObject.name}) does not have component of type {typeof(T).Name}");
            RemoveObject(gameObject);
            return null;
        }

        public static GameObject CreateObject(Transform parent, GameObject prefab, bool copyParentPos = false,
            bool worldPositionStays = false, bool resetScale = false, Action<GameObject> onInstantiate = null)
        {
            if (prefab == null)
                return null;
        ...
        public static void Remove<T>(T target, bool worldPositionStay = false) where T : MonoBehaviour
        {
            if (target is IPoolableBehaviour poolableBehaviour)
            {
                poolableBehaviour.OnPoolRemove();
            }
            RemoveObject(target.gameObject, worldPositionStay: worldPositionStay);
        }
        public static void RemoveObject(GameObject gameObject, bool removeChildren = false,
            bool worldPositionStay = false)
        {
            if (gameObject == null)
            {
                return;
            ...
            foreach (GameObject o in pool)
            {
                if (gameObject == o)
                {
                    Debug.LogError("duplicate object in pool");
                    // Debugger.LogRequest();
                }
            }
#endif
        ...
        public static void RemoveObjects(List<GameObject> objects)
        {
            if (objects == null)
            {
                return;
