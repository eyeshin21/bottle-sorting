...
namespace Anvil
{
    public static class InternetHelper
    {
        ...
        public static bool HasConnection
        {
            ...
            set
            {
                if (value != _hasConnection)
                {
                    ...
                    Log.Debug($"Connection changed: {_hasConnection} => {value}, lastTime: {TimeHelper.SecondsFromStart.ToHMSString()}");
#endif
        ...
        static Action<bool> _checkConnectionCallback;

        /// <summary>
        /// Called from Manager.
        /// </summary>
        public static void Init(Action callback, int timeout = -1)
        {
            InitTracker.Track("InternetHelper");
#if DEBUG_INTERNET_HELPER
            Log.Debug("InternetChecker.Init");
#endif
            _isChecking = true;
            MonoBehaviourMessageForwarder.CommonMessageForwarder.StartCoroutine(CheckConnectionCoroutine(hasConnection =>
            {
                _isChecking = false;
            ...
            MonoBehaviourMessageForwarder.RegisterCommonUpdate(Update);
            MonoBehaviourMessageForwarder.RegisterCommonPauseEvent((pause) =>
            {
                if (!pause)
                {
                    ForceCheck();
        ...
        {
#if DEBUG_INTERNET_HELPER
            Log.Debug("InternetChecker.ForceCheck");
#endif
            _checkDelayTime = 0;
            Update();
        }
        ...
        static void Update()
        {
            ...
            {
                _isChecking = true;
                Anvil.Manager.Instance.StartCoroutine(CheckConnectionCoroutine(hasConnection =>
                {
                    _isChecking = false;
        ...
        public static IEnumerator CheckConnectionCoroutine(Action<bool> callback, int timeout = -1)
        {
        ...
        public static void OnGUIDebug()
        {
            ...
            GUILayout.Label($"hasConnection: {_hasConnection}, lastTime: {PrettyTimeFormater.FormatTime((int)_lastTime, TimeFormat.HHMMSS)}");
            ...
            else
            {
                ...
                {
                    GUILayout.Label($"checkDelayTime: {PrettyTimeFormater.FormatTime((int)_checkDelayTime, TimeFormat.HHMMSS)}");
