...
namespace Anvil.Legacy
{
    ...
    [System.Serializable]
    public class CubicBezierConfig2D
    {


        [SerializeField] AnchorCalculationType anchorType = AnchorCalculationType.WordSpace;
        ...
        {
            if (anchorType == AnchorCalculationType.LocalSpace)
            {
                GetControlsPositionWithLocalNormal(startPos, endPos, out controlPos1, out controlPos2);
                return;
            }else if (anchorType == AnchorCalculationType.WordSpace)
            {
            ...
            else
            {
                ...
                Debug.LogError($"[Cubic bezier config] anchor of type {anchorType} is not supported");
            }
...
#endif
        public CubicBezierConfig2D Clone()
        {
            CubicBezierConfig2D clone = new CubicBezierConfig2D();
