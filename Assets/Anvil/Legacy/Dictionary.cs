...
namespace Anvil
{
    public class Dictionary : System.Collections.Generic.Dictionary<string, string>
    {
        ...
        public Dictionary Deserialize(string json, bool isSimpleJson = false, bool keepDoubleQuotes = false)
        {
            Clear();
                    ...
                    else
                    {
                        ...
                        if (isStringValue)
                        {
                            if (c == '\"')
                            {
                                ...
                                {
                                    value = json.Substring(valueStartIndex, i - valueStartIndex + 1); // Get ""
                                }
                            }
                        }
                        else
        ...
        public static void DeserializeWithQuote(string json, Action<Dictionary> callback, bool simpleJson = false)
        {
            var sharedDict = Shared.Dictionary;
            var dict = sharedDict.Value;
            dict.Deserialize(json, simpleJson, true);
            callback?.Invoke(dict);
        ...
        /// Returns key1: value1,\nkey2: value2,\n...
        /// </summary>
        public static string ToLogString(Action<Dictionary> callback)
        {
            var sharedDict = Shared.Dictionary;
            var dict = sharedDict.Value;
            callback?.Invoke(dict);
        ...
        public static string ToString(Action<Dictionary> callback, char separator = '\n')
        {
            ...
            if (dict.Count > 0)
            {
                foreach (var entry in dict)
                {
                    ...
                    {
                        sb = sb.Append($",{separator}{entry.Key}:{entry.Value}");
                    }
                }
                sb.Append("}");
            }
