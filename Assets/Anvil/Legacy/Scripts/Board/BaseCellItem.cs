...
{
    public abstract class BaseCellItem<TBoard, TCell, TItem> : BaseCell<TBoard, TCell>
        where TBoard : IBoard<TBoard, TCell>
        where TCell : ICell<TBoard, TCell>
        where TItem : class, IItem<TCell>
    {
        ...
        public override bool Empty => !HasItem();
        ...
        protected TItem GetItemTopDown(AcceptFunc<TItem> acceptFunc)
        {
            if (_items != null)
            {
                for (int i = _items.Length - 1; i >= 0; i--)
        ...
        protected void Clear(Callback<TItem> callback)
        {
            if (_items != null)
            {
                for (int i = _items.Length - 1; i >= 0; i--)
